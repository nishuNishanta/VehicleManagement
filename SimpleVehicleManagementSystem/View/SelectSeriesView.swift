            ...
            VStack(spacing: 0) {
                ...
                List {
                    Section(header: Text(createVM.currentSelectionLabel)
                        ...
                        ForEach(createVM.filteredSeries) { series in
                            // NavigationLink: tapping a brand -> Series screen
                            NavigationLink(
                                destination: SelectYearView(createVM: createVM)
                                    .onAppear {
                                        // Set the brand as soon as we navigate
                                        createVM.selectedSeries = series
                                    }
                            ) {
                                HStack {
                                    Text(series.name)
